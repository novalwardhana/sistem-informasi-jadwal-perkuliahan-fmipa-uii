	...
	public function getListCplDetail() {
		...
		if(!empty($getMataKuliahCreateCPL)) {
			foreach ($getMataKuliahCreateCPL as $row) {
				...
				$nestedData['kontribusi'] = "<input type='number' step='0.01' min='0' max='100' id='kontribusi-$row->id' name='kontribusi-$row->id' class='form-control' style='text-align:right; width: 100%' >";;
