...
class MasterKelas extends CI_Controller {
    ...
    public function __construct() {
		...
		$this->load->model('MasterKelasModel');
		$this->masterKelasModel=$this->MasterKelasModel;
    }

    public function index() {
        $data = array();
		$data['title'] = 'SIJP - Master Kelas';
		$this->load->view('masterKelas/list', $data);
    }
    ...
    public function getData() {
		...
		$totalData = $this->masterKelasModel->getTotalData();
		...
		$listData = $this->masterKelasModel->getListData($params);
		$totalFilteredData=$this->masterKelasModel->getTotalFilteredData($params);
		...
		if(!empty($listData)) {
			foreach ($listData as $row) {
				...
				$nestedData['aksi'] = "
					<a href='".base_url('master-kelas/update?id=').$row->id."'>
						<button class='btn btn-sm btn-primary'><i class='fa fa-pencil'></i></button>
					</a>
					<button class='btn btn-sm btn-danger' data-href='".base_url('master-kelas/delete?id=').$row->id."' data-toggle='modal' data-target='#confirm-delete'>
        ...
        switch ($simpan) {
            case true:
                try {
                    ...
                    $result = $this->masterKelasModel->create($data);
                    if ($result != 1) {
    ...
    public function delete() {
		try {
			...
			$params["id"] = $id;
			$result = $this->masterKelasModel->delete($params);
			if ($result != 1) {
        ...
        switch ($simpan) {
            case true:
                try {
                    $data["id"] = $params["id"];
					$data["kode"] = $params["kode"];
                    $result = $this->masterKelasModel->update($data);
					if ($result != 1) {
						throw new Exception($result["message"]);
					}
					$response = array();
            ...
            case false;
                ...
                $data["dataKelas"] = $this->masterKelasModel->getMasterKelasByID($id);
