...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    <?php
      $this->load->view('masterPeriode/updateMain');
    ?>
  </div>
	<?php
...
<script type="text/javascript">
	$(document).ready(function () {
		...
		$(".menu-sidebar-master-periode").addClass('active');

        //
        let tahunAkademik = "<?php echo $dataPeriode->tahun_akademik ?>"
        $(".selectTahunAkademik").val(tahunAkademik).trigger("change")
        let semester = "<?php echo $dataPeriode->semester ?>"
        ...
        document.forms["formData"].addEventListener("submit", (event) => {
            ...
            let tahunAkademik = document.forms["formData"]["tahun_akademik"].value
            let semester = document.forms["formData"]["semester"].value
            const dataBody = {
                "id": id,
            ...
            let url = "<?php echo base_url('master-periode/update') ?>"
            $.ajax({
                headers: {
                ...
                success: function(result) {
                    if (result.code === 200) {
                        $.toaster({ message : 'Berhasil update data', title : 'Success', priority : 'success' });
                                setTimeout(function(){
                            let redirectURL = "<?php echo base_url('master-periode') ?>"
